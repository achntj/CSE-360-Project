 ...
 * 
 * <p>
 * Description: This class provides the user interface for the home page that
 * users are redirected to after login. The layout is role-specific, and users
 * can log out from this page.
 * </p>
 * 
 * <p>
...
public class InstructorHomePage {
	 ...
	 * logout button. It also handles the logout action and redirection to the login
	 * page.
	 * 
	...
	public InstructorHomePage(Stage primaryStage, DatabaseHelper databaseHelper, String email, String role) {
		...
		/*
		 * groupButton.setOnAction(event -> { try { databaseHelper.ensureConnection();
		 * String id = databaseHelper.getUserIdFromEmail(email); // Creates and
		 * redirects to new GroupAccessPage and passes in primary stage and database
		 * helper for usage GroupAccessPage groupAccessPage = new
		 * GroupAccessPage(primaryStage, databaseHelper, id, email, role); Scene
		 * groupAccessScene = new Scene(groupAccessPage.getGroupAccessLayout(), 400,
		 * 300); primaryStage.setScene(groupAccessScene); } catch (Exception e) {
		 * e.printStackTrace();
		 * System.out.println("No user id found with the current email"); } });
		 **/
