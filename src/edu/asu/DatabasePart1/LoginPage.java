 ...
 * 
 * <p>
 * Description: This class provides the user interface for the login page,
 * allowing users to enter their email and password to log in, and handles the
 * redirection based on user roles and account status.
 * </p>
 * 
 * <p>
...
public class LoginPage {
	...
	/************
	 * This constructor initializes the login page and sets up all of the components
	 * in the graphical interface, including buttons, labels, and text fields. It
	 * also manages button actions for logging in and registering.
	 * 
	...
	public LoginPage(Stage primaryStage, DatabaseHelper databaseHelper) {
				...
				if (databaseHelper.login(email, password)) {
					if (databaseHelper.isOneTimePassword(email)) {
						...
						// Redirect to the user home page based on the user's role
							...
							/*
							 * StudentHomePage studentHomePage = new StudentHomePage(primaryStage,
							 * databaseHelper, email, "student"); Scene studentScene = new
							 * Scene(studentHomePage.getStudentHomeLayout(), 400, 300);
							 * primaryStage.setScene(studentScene);
							 **/
						} else {
		...
		// Adds functionality for the 'Register' button to redirect to the registration
		// page
