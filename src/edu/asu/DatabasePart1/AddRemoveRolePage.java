 ...
 * 
 * <p>
 * Description: Setup for an interactive JavaFX page that allows admin users to
 * add or remove roles from specific users in the database
 * </p>
 * 
 * <p>
 ...
 * @version 2.00 2024-10-30 Project Phase 2 Add and Remove Role Page
 * 
 */
...
public class AddRemoveRolePage {
	 ...
	 * presented for the AddRemoveRolePage. It sets up the alignment and text fields
	...
	public AddRemoveRolePage(Stage primaryStage, DatabaseHelper databaseHelper, String email) {
		...
		addRoleButton.setOnAction(event -> {
			...
			// If the role or email is empty, displays error message and alerts user to
			// provide one
			...
			// Adds the role to the entered user and alerts the current user if it went
			// successfully
			try {
		...
		removeRoleButton.setOnAction(event -> {
			...
			// Removes the role from the entered user and alerts the current user if it went
			// successfully
			try {
		...
		backButton.setOnAction(event -> {
			// Creates a new home with the primary stage and database helper and returns to
			// the admin scene
